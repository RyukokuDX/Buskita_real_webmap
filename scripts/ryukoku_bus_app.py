...
class RyukokuBusApp:
    ...
    def is_bus_at_campus(self, bus, campus_coords, threshold_km=0.2):
        ...
        distance = self.calculate_distance(
            bus["lat"], bus["lng"], campus_lat, campus_lng
        )
    ...
    def find_buses_near_campus(self):
        ...
        for bus in buses:
            if self.is_bus_at_campus(bus, campus_coords):
                # 追加情報を計算
                distance = self.calculate_distance(
                    bus["lat"], bus["lng"], campus_coords[0], campus_coords[1]
                )

                bus_info = bus.copy()
                bus_info["distance_to_campus"] = round(
                    distance * 1000)  # メートル単位
    ...
    def get_next_buses(self, route_name=None):
        ...
        routes_to_check = [
            route_name] if route_name else RYUKOKU_BUS_ROUTES.keys()
        ...
        for route in routes_to_check:
            ...
            for time_str in schedule:
                try:
                    ...
                    # 現在時刻より後のバスを検索
                    if bus_hour > current_hour or (
                        bus_hour == current_hour and bus_minute > current_minute
                    ):
                        minutes_until = (bus_hour - current_hour) * 60 + (
                            bus_minute - current_minute
                        )
...
def main():
    ...
    for bus in status["next_buses"]:
        print(
            f"  {bus['route']}: {bus['scheduled_time']} ({bus['minutes_until']}分後)")
    ...
    if status["delay_info"]:
        for route, delays in status["delay_info"].items():
            ...
            for delay in delays:
                print(
                    f"    {delay['scheduled']} → {delay['actual']} ({delay['status']}, {delay['delay_minutes']}分遅延)"
                )
    else:
        print("  現在遅延情報はありません")
