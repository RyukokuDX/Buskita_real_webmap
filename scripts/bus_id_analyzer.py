...
class BusIDAnalyzer:
            ...
            if result:
                ...
                if len(buses) > 0:
                    ...
                    for i, bus in enumerate(buses[:3], 1):
                        print(
                            f"   バス{i}: {json.dumps(bus, ensure_ascii=False)}")
    ...
    def test_get_buses_with_parameters(self):
        ...
        test_patterns = [
            ...
            {
                "name": "JR北海道",
                "data": {"language": 1, "siteId": 1, "companyNo": "jhb"},
            },
            # 位置情報パラメータ
            {
                "name": "位置指定",
            ...
            {
                "name": "workNo追加",
                "data": {"language": 1, "siteId": 9, "workNo": "48385"},
            },
            # その他のパラメータ
            {"name": "routeNo", "data": {"language": 1, "siteId": 9, "routeNo": "1"}},
            {"name": "busStopNo", "data": {
                "language": 1, "siteId": 9, "busStopNo": 1}},
        ]
        ...
        for pattern in test_patterns:
            print(f"\n🔍 {pattern['name']}")
            print(
                f"   パラメータ: {json.dumps(pattern['data'], ensure_ascii=False)}")
        ...
        if buses:
            ...
            if potential_id_fields:
                ...
                for field in potential_id_fields:
                    ...
                    print(
                        f"  {field}: {len(unique_ids)}個のユニークID = {unique_ids[:10]}"
                    )  # 最初の10個
    ...
    def test_different_time_periods(self):
        ...
        for time_slot in time_slots:
            ...
            result = self._make_request(
                "get-buses",
                {"language": 1, "siteId": 9, "time": time_slot, "date": current_date},
            )
...
def main():
    ...
    # 4. 全サイト分析
    analyzer.comprehensive_site_analysis()
